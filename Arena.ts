...
export class Arena {
    public static tournament(hero1: Hero, hero2: Hero) {
        ...
        while (hero1.getHealthPoints() > 0 && hero2.getHealthPoints() > 0) {
            ...
            let hasHero1AbilityActivated: boolean = this.isHeroAbilityActivated(hero1);
            let hasHero2AbilityActivated: boolean = this.isHeroAbilityActivated(hero2);
    ...
    private static attack(attaker: Hero, defender: Hero): void {
        if (attaker.getWeapon().hasItCanUse(attaker)) {
            ...
            console.log("%c" + defender.getName() + " has sufferd: " + defender.sufferDamege(initialDamage).toFixed(2) + " damage", "color: red");
    ...
    private static isHeroAbilityActivated(hero: Hero): boolean {
        if (Math.random() < 0.1) {
